<template>
    <div id="cinema-detail">
      ...
      <div class="movie-info" v-for="(item,index) in hasMovieInfo" :key="index" v-show="movieIndex===Number(index)">
        ...
        <span class="main"><span class="name">{{item.name}}</span><span class="score"><span class="num" v-if="item.score">{{Number(item.score).toFixed(1)}}分</span><span v-else style="font-size: .28rem">暂无评分</span></span></span>
        <span class="intro">
      ...
      <div class="ticket-container">
        <div class="item" v-for="(item,index) in movieDaySchedule" :key="index">
          ...
          <div class="center">
            ...
            <div class="price">{{Number(item.price).toFixed(2)}}元</div>
          </div>
          <div class="right">
    ...
    export default {
        ...
        data(){
          return{
            ...
            movieIndex:0,
            //服务器地址
            server: this.$busData.backendUrl,
            selectedId:0,
            items:[],
            options:{
              activeColor: '#dd2727',
